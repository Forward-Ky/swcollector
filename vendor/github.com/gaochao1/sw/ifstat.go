...
const (
	...
	ifdescriptionOid             = "1.3.6.1.2.1.31.1.1.1.18"
	ifdescriptionOidPrefix       = ".1.3.6.1.2.1.31.1.1.1.18."
...
func ListIfStats(ip, community string, timeout int, ignoreIface []string, retry int, limitConn int, ignorePkt bool, ignoreOperStatus bool, ignoreBroadcastPkt bool, ignoreMulticastPkt bool, ignoreDiscards bool, ignoreErrors bool, ignoreUnknownProtos bool, ignoreOutQLen bool) ([]IfStats, error) {
	...
	chIfDescriptionList := make(chan []gosnmp.SnmpPDU)
	...
	time.Sleep(5 * time.Millisecond)
	limitCh <- true
	go ListIfDescription(ip, community, timeout, chIfDescriptionList, retry, limitCh)
	time.Sleep(5 * time.Millisecond)
	...
	ifDescriptionList :=<- chIfDescriptionList
	if ignoreOperStatus == false {
	...
	if len(ifNameList) > 0 && len(ifInList) > 0 && len(ifOutList) > 0 {
		...
		for _, ifNamePDU := range ifNameList {
			...
			if check {
				...
				for ti, ifdescriptionPDU := range ifDescriptionList {
					if strings.Replace(ifdescriptionPDU.Name, ifdescriptionOidPrefix, "", 1) == ifIndexStr {
						ifStats.IfDescription = ifDescriptionList[ti].Value.(string)
						break
					}
				}
...
func ListIfDescription(ip, community string, timeout int, ch chan []gosnmp.SnmpPDU, retry int, limitCh chan bool) {
	RunSnmpRetry(ip, community, timeout, ch, retry, limitCh, ifdescriptionOid)
}
...
func RunSnmpRetry(ip, community string, timeout int, ch chan []gosnmp.SnmpPDU, retry int, limitCh chan bool, oid string) {
	...
	<-limitCh
	//if i:=strings.Contains(oid, "1.3.6.1.2.1.31.1.1.1.18");i{
